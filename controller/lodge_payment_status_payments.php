...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $payment_lodger = $_SESSION['username']; // Admin's username from session
    date_default_timezone_set('Asia/Manila');
    $lodged_date = date('Y-m-d H:i:s'); // Current timestamp
    ...
    $stmt = $conn->prepare("
        ...
        WHERE id = ?
    ");
    $stmt->bind_param("sssi", $new_status,  $payment_lodger, $lodged_date, $invoice_id);
    ...
    header("Location: ../payments/lodge_payment.php?id=" . $invoice_id);
    exit;
} else {
